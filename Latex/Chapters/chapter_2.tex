...
Machine learning is a powerful tool for identifying patterns in complex datasets. In particle physics, it can be used to detect signals from background noise in large detector datasets. Specifically, for detecting IBD signals from background, machine learning algorithms can analyze the data for patterns indicative of an IBD event and distinguish them from background noise.

\subsection{Supervised Learning}
Supervised learning is a fundamental aspect of machine learning, where algorithms are trained using labeled datasets. The goal is to develop a function that accurately links input data to corresponding outputs. In the context of this thesis, the focus is on binary classification in the JUNO experiment. The task is to determine whether a given event is an inverse beta decay event or a background event based on inherent features.
...
Gradient Boosting is a machine learning algorithm that stems from the concept of boosting, with the application of gradient descent methodology. Its goal is to produce a robust predictive model through the combination of multiple weak learners, decision trees.
...
Neural Networks (NNs) are computational models inspired by the interconnected structure of the human brain. Each artificial neuron in an NN mimics the behavior of a biological neuron.
...
An Artificial Neural Network (ANN) consists of interconnected layers of neurons, including an input layer, one or more hidden layers, and an output layer. Each layer is fully connected to the next layer.
A graphical representation of an Artificial Neural Network and a single neuron is presented in Figure \ref{fig:nn} and Figure \ref{fig:neuron}.
